...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfloating, isalwaysontop, isurgent, neverfocus, oldstate, isfullscreen, issticky, isgame;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
getsystraywidth()
{
	...
	Client *i;
	if(showsystray)
		for(i = systray->icons; i; w += i->w + 5, i = i->next) ;
	return w ? w + 5 : 1;
}
...
setmfact(const Arg *arg)
{
    ...
    if (arg->f <= -1.0) {
        selmon->mfact = selmon->pertag->mfacts[selmon->pertag->curtag] = DEFAULT_MFACT;
        arrange(selmon);
        return;
    }
